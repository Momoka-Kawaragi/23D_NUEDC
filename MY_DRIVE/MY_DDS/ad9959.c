#include "ad9959.h"

/* 强制 IO 定义（由 CubeMX 生成的 GPIO 宏提供端口与引脚） */
driverIO SDIO0 = {SDIO0_9959_GPIO_Port, SDIO0_9959_Pin};
driverIO SDIO1 = {SDIO0_9959_GPIO_Port, SDIO1_9959_Pin};
driverIO SDIO2 = {SDIO2_9959_GPIO_Port, SDIO2_9959_Pin};
driverIO SDIO3 = {SDIO3_9959_GPIO_Port, SDIO3_9959_Pin};
driverIO PDC = {PDC_9959_GPIO_Port, PDC_9959_Pin};
driverIO RST = {RST_9959_GPIO_Port, RST_9959_Pin};
driverIO SCLK = {SCLK_9959_GPIO_Port, SCLK_9959_Pin};
driverIO CS = {CS_9959_GPIO_Port, CS_9959_Pin};
driverIO UPDATE = {UPDATE_9959_GPIO_Port, UPDATE_9959_Pin};
driverIO PS0 = {PS0_9959_GPIO_Port, PS0_9959_Pin};
driverIO PS1 = {PS1_9959_GPIO_Port, PS1_9959_Pin};
driverIO PS2 = {PS2_9959_GPIO_Port, PS2_9959_Pin};
driverIO PS3 = {PS3_9959_GPIO_Port, PS3_9959_Pin};

uint8_t CSR_DATA0[1] = {0x10}; // 选择 CH0
uint8_t CSR_DATA1[1] = {0x20}; // 选择 CH1
uint8_t CSR_DATA2[1] = {0x40}; // 选择 CH2
uint8_t CSR_DATA3[1] = {0x80}; // 选择 CH3

uint8_t CSR_DATA[4] = {0x10, 0x20, 0x40, 0x80}; // 选择 CH0/CH1/CH2/CH3

uint8_t FR2_DATA[2] = {0x00, 0x00}; // 默认值 0x0000

/*
uint8_t LSRR_DATA[2] = {0x00, 0x00};            // 默认值 = 0x----
uint8_t RDW_DATA[4] = {0x00, 0x00, 0x00, 0x00}; // 默认值 = 0x--------
uint8_t FDW_DATA[4] = {0x00, 0x00, 0x00, 0x00}; // 默认值 = 0x--------
*/

// uint32_t SinFre[4] = {10000000, 10000000, 200000000, 40000};
uint32_t SinFre[4] = {1000, 1000, 1000, 1000};
uint32_t SinAmp[4] = {1023, 1023, 1023, 1023};
// uint32_t SinPhr[4] = {0, 4095, 4095 * 3, 4095 * 2};
uint32_t SinPhr[4] = {0, 0, 0, 0};

/**
 * @brief 初始化 AD9959
 * @note  调用前需确保 GPIO 已由 CubeMX 初始化完成
 *        本函数只写入寄存器，不主动调用 AD9959_IO_Update()
 */
void Init_AD9959(void)
{
    uint8_t FR1_DATA[3] = {0xD3, 0x00, 0x00}; // PLL 倍频系数配置（20 倍频）
    uint8_t CFR_DATA[3] = {0x00, 0x03, 0x00}; // 默认值 0x000302
    InitIO_9959();
    InitReset();

    WriteData_AD9959(FR1_ADD, 3, FR1_DATA);
    WriteData_AD9959(CFR_ADD, 3, CFR_DATA);

    Write_Phase(3, SinPhr[3]);
    Write_Phase(0, SinPhr[0]);
    Write_Phase(1, SinPhr[1]);
    Write_Phase(2, SinPhr[2]);

    Write_Frequence(0, SinFre[0]);
    Write_Frequence(1, SinFre[1]);
    Write_Frequence(2, SinFre[2]);
    Write_Frequence(3, SinFre[3]);

    Write_Amplitude(3, SinAmp[3]);
    Write_Amplitude(0, SinAmp[0]);
    Write_Amplitude(1, SinAmp[1]);
    Write_Amplitude(2, SinAmp[2]);
}

/**
 * @brief AD9959 软件延时（空循环）
 * @param length 延时循环次数（粗略延时，与主频有关）
 */
void delay_9959(uint32_t length)
{
    length = length * 12;
    while (length--)
        ;
}

/**
 * @brief 初始化 AD9959 IO 引脚默认电平
 * @note  用于保证上电时序与初始状态一致
 */
void InitIO_9959(void)
{

    WRT(PDC, 0);
    WRT(CS, 1);
    WRT(SCLK, 0);
    WRT(UPDATE, 0);
    WRT(PS0, 0);
    WRT(PS1, 0);
    WRT(PS2, 0);
    WRT(PS3, 0);
    WRT(SDIO0, 0);
    WRT(SDIO1, 0);
    WRT(SDIO2, 0);
    WRT(SDIO3, 0);
}

/**
 * @brief 复位 AD9959（产生 RST 脉冲）
 */
void InitReset(void)
{
    WRT(RST, 0);
    delay_9959(1);
    WRT(RST, 1);
    delay_9959(30);
    WRT(RST, 0);
}

/**
 * @brief 产生 IO UPDATE 脉冲，使寄存器新配置生效
 * @note  AD9959 的多数寄存器写入后需要 UPDATE 才会更新到输出通道
 */
void AD9959_IO_Update(void)
{
    WRT(UPDATE, 1);
    delay_9959(1);
    WRT(UPDATE, 0);
}

/**
 * @brief 向 AD9959 写寄存器数据（仅写入，不自动 UPDATE）
 * @param RegisterAddress 寄存器地址
 * @param NumberofRegisters 写入字节数
 * @param RegisterData 写入数据指针（按字节顺序写出）
 */
void WriteData_AD9959(uint8_t RegisterAddress, uint8_t NumberofRegisters, uint8_t *RegisterData)
{
    uint8_t ControlValue = 0;
    uint8_t ValueToWrite = 0;
    uint8_t RegisterIndex = 0;
    uint8_t i = 0;

    ControlValue = RegisterAddress;
    /* 写地址 */
    WRT(SCLK, 0);
    WRT(CS, 0);
    for (i = 0; i < 8; i++)
    {
        WRT(SCLK, 0);
        if (0x80 == (ControlValue & 0x80))
            WRT(SDIO0, 1);
        else
            WRT(SDIO0, 0);
        WRT(SCLK, 1);
        ControlValue <<= 1;
        delay_9959(2);
    }

    WRT(SCLK, 0);
    /* 写数据：每个字节从高位开始发送（高位先出） */
    for (RegisterIndex = 0; RegisterIndex < NumberofRegisters; RegisterIndex++)
    {
        ValueToWrite = RegisterData[RegisterIndex];
        for (i = 0; i < 8; i++)
        {
            WRT(SCLK, 0);
            if (0x80 == (ValueToWrite & 0x80))
                WRT(SDIO0, 1);
            else
                WRT(SDIO0, 0);
            WRT(SCLK, 1);
            ValueToWrite <<= 1;
            delay_9959(2);
        }
        WRT(SCLK, 0);
    }
    WRT(CS, 1);
}

/**
 * @brief 设置指定通道频率（写入 CFTW0，不自动 UPDATE）
 * @param Channel 通道号（0~3）
 * @param Freq 频率（Hz，1~500000000）
 */
void Write_Frequence(uint8_t Channel, uint32_t Freq)
{
    if (Freq > 500000000 || Freq < 1)
    {
        Freq = 114514;
        AD9959_error();
    }

    uint8_t CFTW0_DATA[4] = {0x00, 0x00, 0x00, 0x00};
    Freq2Word(Freq, CFTW0_DATA);
    Channel_Select(Channel);
    WriteData_AD9959(CFTW0_ADD, 4, CFTW0_DATA);
}

/**
 * @brief 设置指定通道幅度（写入 ACR，不自动 UPDATE）
 * @param Channel 通道号（0~3）
 * @param Ampli 幅度（0~1023）
 */
void Write_Amplitude(uint8_t Channel, uint16_t Ampli)
{
    // Ampli 的取值范围：0~1023
    if (Ampli > 1023 || Ampli < 0)
    {
        Ampli = 114;
        AD9959_error();
    }
    uint8_t ACR_DATA[3] = {0x00, 0x00, 0x00}; // 默认值 0x--0000
    Amp2Word(Ampli, ACR_DATA);
    Channel_Select(Channel);
    WriteData_AD9959(ACR_ADD, 3, ACR_DATA);
}

/**
 * @brief 设置指定通道相位（写入 CPOW0，不自动 UPDATE）
 * @param Channel 通道号（0~3）
 * @param Phase 相位（度，0~359）
 */
void Write_Phase(uint8_t Channel, uint16_t Phase)
{
    // Phase_max = 16383
    if (Phase > 359 || Phase < 0)
    {
        Phase = 0;
        AD9959_error();
    }
    uint8_t CPOW0_DATA[2] = {0x00, 0x00};
    Phase2Word(Phase, CPOW0_DATA);
    Channel_Select(Channel);
    WriteData_AD9959(CPOW0_ADD, 2, CPOW0_DATA);
}

/**
 * @brief 一键配置通道：频率/幅度/相位（不自动 UPDATE）
 * @note  调用后需再执行 AD9959_IO_Update() 才会生效
 * @param Channel 通道号（0~3）
 * @param Ampli 幅度（0~1023）
 * @param Phase 相位（度，0~359）
 * @param Freq 频率（Hz）
 */
void AD9959_SetChannel(uint8_t Channel, uint16_t Ampli, uint16_t Phase, uint32_t Freq)
{
    Write_Frequence(Channel, Freq);
    Write_Amplitude(Channel, Ampli);
    Write_Phase(Channel, Phase);
}

/**
 * @brief 选择当前操作通道（写 CSR）
 * @param Channel 通道号（0~3）
 */
void Channel_Select(uint8_t Channel)
{
    if (Channel < 0 || Channel > 3)
    {
        AD9959_error();
        return;
    }
    else
        WriteData_AD9959(CSR_ADD, 1, CSR_DATA + Channel);
}

/**
 * @brief AD9959 错误处理函数（可按项目需要实现）
 * @note  当参数越界或状态异常时会调用此函数
 */
void AD9959_error(void)
{
}

/**
 * @brief 线性扫频（使用 AD9959 内部扫频功能）
 * @note  该函数会配置相关寄存器与 PS 引脚状态；是否生效与硬件连接有关
 * @param Channel 通道号（0~3）
 * @param Start_Freq 起始频率（Hz）
 * @param Stop_Freq 终止频率（Hz）
 * @param Step 步进频率（Hz）
 * @param time 每步持续时间（微秒，1~2048）
 * @param NO_DWELL 是否开启 no-dwell（1 开启 / 0 关闭）
 */
void Sweep_Frequency(uint8_t Channel, uint32_t Start_Freq, uint32_t Stop_Freq, uint32_t Step, uint32_t time, uint8_t NO_DWELL)
{
    Channel_Select(Channel);
    switch (Channel)
    {
    case 0:
        WRT(PS0, 0);
    case 1:
        WRT(PS1, 0);
    case 2:
        WRT(PS2, 0);
    case 3:
        WRT(PS3, 0);
    }

    uint8_t CFR_GET[3] = {0x00, 0x00, 0x00};
    ReadData_AD9959(CFR_ADD, 3, CFR_GET);
    uint8_t CFR_DATA_Freq[3] = {CFR_GET[0] | 0x80, CFR_GET[1] | 0x40, CFR_GET[2] | 0x00};
    // 关闭/开启 no-dwell（CFR 对应位）
    if (NO_DWELL)
        CFR_DATA_Freq[1] = CFR_DATA_Freq[1] | 0x80;

    uint8_t FR1_DATA_sweep[3] = {0xD3, 0x00, 0x00};
    WriteData_AD9959(CFR_ADD, 3, CFR_DATA_Freq);
    WriteData_AD9959(FR1_ADD, 3, FR1_DATA_sweep);

    uint32_t Start_Freq_word, Stop_Freq_Word, Step_Word;
    uint8_t Time_word;
    Start_Freq_word = (uint32_t)Start_Freq * 8.589934592;
    Stop_Freq_Word = (uint32_t)Stop_Freq * 8.589934592;
    Step_Word = (uint32_t)Step * 8.589934592;
    Time_word = (uint8_t)time / 8.0;

    uint8_t CFTW0_DATA_START[4] = {0x00, 0x00, 0x00, 0x00};
    uint8_t CFTW0_DATA_STOP[4] = {0x00, 0x00, 0x00, 0x00};
    uint8_t RDW_DATA[4] = {0x00, 0x00, 0x00, 0x00};

    CFTW0_DATA_START[3] = (uint8_t)Start_Freq_word;
    CFTW0_DATA_START[2] = (uint8_t)(Start_Freq_word >> 8);
    CFTW0_DATA_START[1] = (uint8_t)(Start_Freq_word >> 16);
    CFTW0_DATA_START[0] = (uint8_t)(Start_Freq_word >> 24);
    WriteData_AD9959(CFTW0_ADD, 4, CFTW0_DATA_START);

    CFTW0_DATA_STOP[3] = (uint8_t)Stop_Freq_Word;
    CFTW0_DATA_STOP[2] = (uint8_t)(Stop_Freq_Word >> 8);
    CFTW0_DATA_STOP[1] = (uint8_t)(Stop_Freq_Word >> 16);
    CFTW0_DATA_STOP[0] = (uint8_t)(Stop_Freq_Word >> 24);
    WriteData_AD9959(CW1, 4, CFTW0_DATA_STOP);

    RDW_DATA[3] = (uint8_t)Step_Word;
    RDW_DATA[2] = (uint8_t)(Step_Word >> 8);
    RDW_DATA[1] = (uint8_t)(Step_Word >> 16);
    RDW_DATA[0] = (uint8_t)(Step_Word >> 24);
    WriteData_AD9959(RDW_ADD, 4, RDW_DATA);

    uint8_t FDW_DATA[4] = {0x00, 0x00, 0x00, 0x00};
    FDW_DATA[3] = (uint8_t)Step_Word;
    FDW_DATA[2] = (uint8_t)(Step_Word >> 8);
    FDW_DATA[1] = (uint8_t)(Step_Word >> 16);
    FDW_DATA[0] = (uint8_t)(Step_Word >> 24);
    WriteData_AD9959(FDW_ADD, 4, FDW_DATA);

    uint8_t LSRR_DATA[2] = {0x00, 0x00};
    LSRR_DATA[1] = (uint8_t)Time_word;
    LSRR_DATA[0] = (uint8_t)Time_word;
    WriteData_AD9959(LSRR_ADD, 2, LSRR_DATA);

    switch (Channel)
    {
    case 0:
        WRT(PS0, 1);
    case 1:
        WRT(PS1, 1);
    case 2:
        WRT(PS2, 1);
    case 3:
        WRT(PS3, 1);
    }
}

uint32_t Get_Freq(void)
{
    uint8_t data[4] = {0};
    ReadData_AD9959(CFTW0_ADD, 4, data);
    int Freq = (data[0] << 24 | data[1] << 16 | data[2] << 8 | data[3]) / 8.589934592;
    return Freq;
}

/**
 * @brief 读取当前幅度（ACR）
 * @return 幅度值（0~1023）
 */
uint8_t Get_Amp(void)
{
    uint8_t data[3] = {0};
    ReadData_AD9959(ACR_ADD, 3, data);
    return data[2];
}

/**
 * @brief 从 AD9959 读寄存器数据
 * @note  读操作会临时将 SDIO0 配置为输入，读完后恢复为推挽输出
 * @param RegisterAddress 寄存器地址
 * @param NumberofRegisters 读取字节数
 * @param RegisterData 读出数据缓冲区
 */
void ReadData_AD9959(uint8_t RegisterAddress, uint8_t NumberofRegisters, uint8_t *RegisterData)
{
    uint8_t ControlValue = 0;
    uint8_t ValueToRead = 0;
    uint8_t RegisterIndex = 0;
    uint8_t i = 0;
    ControlValue = RegisterAddress | 0x80;
    /* 写地址（读模式：地址最高位 OR 0x80） */
    WRT(CS, 0);
    WRT(SCLK, 0);
    delay_9959(0x20);
    for (i = 0; i < 8; i++)
    {
        WRT(SCLK, 0);
        if (0x80 == (ControlValue & 0x80))
            WRT(SDIO0, 1);
        else
            WRT(SDIO0, 0);
        WRT(SCLK, 1);
        ControlValue <<= 1;
        delay_9959(2);
    }

    WRT(SCLK, 0);

    /* SDIO0 切换为输入模式 */
    GPIO_InitTypeDef GPIO_InitStruct = {0};
    GPIO_InitStruct.Pin = SDIO0_9959_Pin;
    GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(SDIO0_9959_GPIO_Port, &GPIO_InitStruct);
    delay_9959(0x20);
    /* 读数据：高位先出 */
    for (RegisterIndex = 0; RegisterIndex < NumberofRegisters; RegisterIndex++)
    {
        ValueToRead = 0;
        for (i = 0; i < 8; i++)
        {
            ValueToRead <<= 1;
            WRT(SCLK, 1);
            if (GET(SDIO0) == 1)
                ValueToRead |= 0x01;
            else
                ValueToRead &= 0xFE;
            WRT(SCLK, 0);
        }
        WRT(SCLK, 0);
        RegisterData[RegisterIndex] = ValueToRead;
    }
    delay_9959(0x20);
    WRT(CS, 1);
    GPIO_InitStruct.Pull = GPIO_PULLDOWN;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    HAL_GPIO_Init(SDIO0_9959_GPIO_Port, &GPIO_InitStruct);
}

/**
 * @brief 停止 AD9959 输出（进入掉电/停止模式）
 */
void Stop_AD9959(void)
{
    uint8_t CFR_data[3] = {0};
    ReadData_AD9959(CFTW0_ADD, 3, CFR_data);
    CFR_data[0] = CFR_data[0] | 0x02;
    WriteData_AD9959(CFR_ADD, 3, CFR_data);
}

/**
 * @brief 配置 2FSK（Profile=0/1 两个频率）
 * @param Channel 通道号（0~3）
 * @param f_start Profile=0 时频率（Hz）
 * @param f_stop Profile=1 时频率（Hz）
 */
void SET_2FSK(uint8_t Channel, double f_start, double f_stop)
{
    uint8_t f_startWord[4];
    uint8_t f_stopWord[4];
    uint8_t CFR_data[3] = {0x80, 0x23, 0x30}; // none RU/RD
    uint8_t FR_data[3] = {0xD0, 0x00, 0x00};

    Channel_Select(Channel);

    WriteData_AD9959(FR1_ADD, 3, FR_data);
    WriteData_AD9959(CFR_ADD, 3, CFR_data);

    Freq2Word(f_start, f_startWord);
    Freq2Word(f_stop, f_stopWord);

    WriteData_AD9959(CFTW0_ADD, 4, f_startWord);
    WriteData_AD9959(CW1, 4, f_stopWord);
}

/**
 * @brief 配置 2ASK（Profile=0/1 两个幅度）
 * @param Channel 通道号（0~3）
 * @param f 载波频率（Hz）
 * @param A_start Profile=0 幅度（0~1023）
 * @param A_stop Profile=1 幅度（0~1023）
 */
void SET_2ASK(uint8_t Channel, double f, uint16_t A_start, uint16_t A_stop)
{
    uint8_t fWord[4];
    uint8_t A_startWord[3] = {0x00, 0x00, 0x00};// Profile=0 幅度
    uint8_t A_stopWord[4] = {0x00, 0x00, 0x00, 0x00};// Profile=1 幅度
    uint8_t A_stopWord_temp[3] = {0x00, 0x00, 0x00};/// Profile=1 幅度临时字

    uint8_t CFR_data[3] = {0x40, 0x03, 0x30};// Profile=0 幅度
    uint8_t FR_data[3] = {0xD0, 0x00, 0x00};

    Amp2Word(A_start, A_startWord);
    Amp2Word(A_stop, A_stopWord_temp);

    // A_stopWord_temp[9:0] -> A_stopWord[31:22]
    uint32_t Temp = ((uint32_t)(A_stopWord_temp[1] | 0x03)) << 8 | (uint32_t)(A_stopWord_temp[2]);
    Temp = Temp << 22;
    A_stopWord[0] = (uint8_t)(Temp >> 24);
    A_stopWord[1] = (uint8_t)(Temp >> 22);

    Channel_Select(Channel);

    WriteData_AD9959(FR1_ADD, 3, FR_data);
    WriteData_AD9959(CFR_ADD, 3, CFR_data);

    WriteData_AD9959(ACR_ADD, 3, A_startWord);
    WriteData_AD9959(CW1, 4, A_stopWord);

    Freq2Word(f, fWord);
    WriteData_AD9959(CFTW0_ADD, 4, fWord);
}

void Freq2Word(double f, uint8_t *fWord)
{
    // fWord 4 字节调谐字（高字节在前）
    uint32_t Temp;
    Temp = (uint32_t)f * 8.589934592; // 8.589934592=(2^32)/500000000（假设系统时钟为 500MHz）
    fWord[3] = (uint8_t)Temp;
    fWord[2] = (uint8_t)(Temp >> 8);
    fWord[1] = (uint8_t)(Temp >> 16);
    fWord[0] = (uint8_t)(Temp >> 24);
}

/**
 * @brief 幅度换算为 ACR 写入字
 * @param A 幅度（0~1023）
 * @param AWord 3字节输出（高字节在前）
 */
void Amp2Word(uint16_t A, uint8_t *AWord)
{
    // AWord 3 字节（高字节在前）
    uint16_t Temp;
    Temp = (uint16_t)A | 0x1000; // 使能幅度乘法器
    AWord[2] = (uint8_t)Temp;
    AWord[1] = (uint8_t)(Temp >> 8);
    AWord[0] = 0x00;
}

/**
 * @brief 相位换算为 CPOW0 写入字
 * @param Phase 相位（度，0~359）
 * @param PWord 2字节输出（高字节在前）
 */
void Phase2Word(uint16_t Phase, uint8_t *PWord)
{
    // PWord 2 字节（高字节在前）
    uint16_t Temp = (uint16_t)(45.511111111 * Phase); // 360/2^16 = 45.511111111
    PWord[1] = (uint8_t)Temp;
    PWord[0] = (uint8_t)(Temp >> 8);
}
